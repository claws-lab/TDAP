...
from baselines.greedy.greedy_attack import TGA
    ...
    for target_id in tqdm(range(idx_targets.shape[0])):
        ...
        if (ntargets > 1):
            ...
            for ntg in range(ntargets):
                attacker = TGA(model=victim_model, num_graphs=context, ntg_nodes=2, directed=not(cmd_args.undirected), nnodes=data.max_nodes, nclasses=nclasses,
                            loss_type=cmd_args.loss_type, device=device, args_device=device, large_graph=cmd_args.large_graph)
                ...
                if (cmd_args.constraint == "budget"):
                    ...
                    attacker.attack_budget(mod_graphs, target_label[ntg], target_nds[ntg], cmd_args.budget, disturbance_ratio=cmd_args.tga_disturbance_ratio,
                                        attack_mode=cmd_args.tga_attack_mode, threshold=cmd_args.tga_thresh_prob)
                elif (cmd_args.constraint == "noise"):
                    attacker.attack_noise(mod_graphs, target_label[ntg], target_nds[ntg], cmd_args.epsilon, cmd_args.epsilon1, disturbance_ratio=cmd_args.tga_disturbance_ratio,
                                        attack_mode=cmd_args.tga_attack_mode, threshold=cmd_args.tga_thresh_prob)
                ...
                #     pass 
                # del attacker
                perbs.append(torch.where(attacker.best_adv_example))
            print (total_time)
        else:
            attacker = TGA(model=victim_model, num_graphs=context, ntg_nodes=ntargets*2, directed=not(cmd_args.undirected), nnodes=data.max_nodes, nclasses=nclasses,
                            loss_type=cmd_args.loss_type, device=device, args_device=device, large_graph=cmd_args.large_graph)
            target_time = time.time()
            if (cmd_args.constraint == "budget"):
                attacker.attack_budget(context_graphs, target_label, target_nds, cmd_args.budget, disturbance_ratio=cmd_args.tga_disturbance_ratio,
                                        attack_mode=cmd_args.tga_attack_mode, threshold=cmd_args.tga_thresh_prob)
            elif (cmd_args.constraint == "noise"):
                if (cmd_args.debug):
                    print ("attacking", get_gpu_info(cmd_args.device))
                attacker.attack_noise(context_graphs, target_label, target_nds, cmd_args.epsilon, cmd_args.epsilon1, disturbance_ratio=cmd_args.tga_disturbance_ratio,
                                        attack_mode=cmd_args.tga_attack_mode, threshold=cmd_args.tga_thresh_prob, debug=cmd_args.debug, iter_ub=cmd_args.tga_iter_ub)
                if (cmd_args.debug):
        ...
        if (ntargets > 1) and (cmd_args.seq_tg_attk):
            ...
            perturbations = torch.where(attacker.best_adv_example)
