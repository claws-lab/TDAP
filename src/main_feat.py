...
from src.pgd_attack_feat import FeatTDPGD
        ...
        if (ntargets > 1) and (cmd_args.seq_tg_attk):
            ...
            for ntg in range(ntargets):
                if (cmd_args.method == "pgd"):
                    attacker = FeatTDPGD(model=victim_model, num_graphs=context, ntg_nodes=2, khop=cmd_args.khop, thresh_wt=cmd_args.thresh_wt, args_device=device, inits=cmd_args.inits,
            ...
            if (cmd_args.method == "pgd"):
                attacker = FeatTDPGD(model=victim_model, num_graphs=context, ntg_nodes=ntargets*2, khop=cmd_args.khop, thresh_wt=cmd_args.thresh_wt, large_graph=cmd_args.large_graph,
