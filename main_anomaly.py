...
from netwalk_anomaly import kmeans_cluster, netwalk_clean_graph

def link_sample_anom (perb_edges, graphs):
    edges_t = []
    for t in range(perb_edges.shape[0]):
        rand_edges = graphs[t].edge_index[:, torch.randperm (graphs[t].edge_index.shape[1])]
        num_edges = int(perb_edges[t].sum())
...
if __name__ == '__main__':
    ...
    if (task == "node_classification"):
        ...
        data.link_split(target_snapshot, val_p=0.0, test_p=0.8, sampling=cmd_args.sampling, num_samples=cmd_args.num_samples)
    ...
    for target_id in range(idx_targets.shape[0]):
        ...
        if (ntargets > 1) and (cmd_args.seq_tg_attk):
            ...
            for ntg in range(ntargets):
                if (cmd_args.method == "pgd"):
                    attacker = DPGD(model=victim_model, num_graphs=context, ntg_nodes=2, khop=cmd_args.khop, thresh_wt=cmd_args.thresh_wt, args_device=device, inits=cmd_args.inits,
                ...
                elif (cmd_args.method == "prbcd"):
                    attacker = PRBCD(model=victim_model, num_graphs=context, ntg_nodes=2, khop=cmd_args.khop, large_graph=cmd_args.large_graph, 
                                        directed=not(cmd_args.undirected), nnodes=data.max_nodes, loss_type=cmd_args.loss_type, device=device)
                ...
                if (cmd_args.constraint == "budget"):
                    ...
                    attacker.attack(mod_graphs, target_label[ntg], target_nds[ntg], constraint=cmd_args.constraint,  constr_vars=[cmd_args.epsilon, cmd_args.epsilon1], 
                                    orig_embs=orig_embs, lambda1=cmd_args.lambda1, epochs=cmd_args.num_steps, lr_init=cmd_args.lr_init, use_optim=cmd_args.use_optim)
                elif (cmd_args.constraint == "noise_budget"):
                    attacker.attack(mod_graphs, target_label[ntg], target_nds[ntg], constraint=cmd_args.constraint,  constr_vars=[cmd_args.epsilon, cmd_args.epsilon1], 
                                    orig_embs=orig_embs, lambda1=cmd_args.lambda1, epochs=cmd_args.num_steps, lr_init=cmd_args.lr_init, use_optim=cmd_args.use_optim)
            ...
            if (cmd_args.method == "pgd"):
                attacker = DPGD(model=victim_model, num_graphs=context, ntg_nodes=ntargets*2, khop=cmd_args.khop, thresh_wt=cmd_args.thresh_wt, large_graph=cmd_args.large_graph,
                ...
                attacker = PRBCD(model=victim_model, num_graphs=context, ntg_nodes=ntargets*2, khop=cmd_args.khop, large_graph=cmd_args.large_graph, 
                                    directed=not(cmd_args.undirected), nnodes=data.max_nodes, loss_type=cmd_args.loss_type, device=device)
            target_time = time.time()
            if (cmd_args.constraint == "budget"):
        ...
        _, score_before_perb = kmeans_cluster(orig_embs.cpu().numpy(), train_indices=data.train_mask, 
                                                perb_edges=attacker.adj_changes.cpu().detach().numpy(), target_nds=target_nds, k=5)
        _, score_after_perb = kmeans_cluster(embs.cpu().detach().numpy(), train_indices=data.train_mask, 
                                                perb_edges=attacker.adj_changes.cpu().detach().numpy(), target_nds=target_nds, k=5)
        # exst_links = link_sample_anom(attacker.adj_changes.cpu().detach().numpy(), data.graphs)
        # _, score_after_perb_edges = kmeans_cluster(embs.cpu().detach().numpy(), train_indices=data.train_mask, 
        #                                             perb_edges=exst_links, k=5, target_nds=target_nds, edge_list=True)
        # print (score_after_perb, score_after_perb_edges)
        # anscores.append (score_after_perb + score_after_perb_edges)
        # an_gtruth.append ([1]*len(score_after_perb) + [0]* len(score_after_perb_edges))
        anscores_orig.append (score_before_perb)
        anscores_perb.append (score_after_perb)
        print (np.abs(score_after_perb - score_before_perb))
        ...
        precl_embs = victim_model(mod_graphs, idx_targets=idx_targets) if (cmd_args.large_graph) else victim_model(mod_graphs)
        mod_an_graphs = netwalk_clean_graph (mod_graphs, target_nds, precl_embs.cpu().detach().numpy(), data.train_mask, k=cmd_args.k_anom)
        print ([graph.edge_index.shape for graph in mod_graphs])
        print ([graph.edge_index.shape for graph in mod_an_graphs])
        postcl_embs = victim_model(mod_an_graphs, idx_targets=idx_targets) if (cmd_args.large_graph) else victim_model(mod_an_graphs)
        pred = victim_model.predict(postcl_embs, target_nds)
        try:
            pred_anom_attkd.append(pred.item())
        except:
            pred_anom_attkd.append(pred.detach().cpu().numpy())
    ...
    # print ("Original Anomalous scores:", anscores_orig)
    # print ("Anomalous scores after perturbation:", anscores_perb)
    anscores_perb = np.array(anscores_perb)
    save_dir = f"results_{cmd_args.model_name.lower()}_anom/{cmd_args.dataset}/multi_targets/{cmd_args.method}/{cmd_args.constraint}"
    pickle.dump(anscores_orig, open(f"{save_dir}/anscores_orig_eps{cmd_args.epsilon}.pkl", 'wb'))
    pickle.dump(anscores_perb, open(f"{save_dir}/perb_scores_eps{cmd_args.epsilon}.pkl", 'wb'))
    print ("Anomalous score difference:", np.abs(anscores_perb - anscores_orig).mean())

    anscores = np.array (anscores)
    anlabels = np.array (an_gtruth)
    # print ("AUROC for Netwalk: ", roc_auc_score(anlabels, anscores))
    ...
    if task == 'link_prediction':
        if (ntargets > 1):
            print ("Anom AUCROC after perturbation: {}".format(av_roc_targets(y_targets, np.stack(pred_anom_attkd))))
        else:
            print ("Anom AUCROC after perturbation: {}".format(roc_auc_score(y_targets.reshape(total_edges), pred_anom_attkd)))
    elif task == 'node_classification':
        if (nclasses == 2):
            print ("Anom AUCROC after perturbation: {}".format(roc_auc_score (y_targets, pred_anom_attkd)))
        else:
            print (confusion_matrix(y_targets, np.stack(pred_anom_attkd).argmax(axis=-1)))
            print ("Anom Accuracy after perturbation: {}".format(accuracy_score (y_targets, np.stack(pred_anom_attkd).argmax(axis=-1))))
